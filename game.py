...
y="" #stores symbol for present turn
x=2  #decides which player should play
...
def define_sign(number): #parameter:box number
    global x,y,f
    ...
    if number==1:
        if x%2==0:
            if 1 in player_1 or 1 in player_2:
                f=1 #entry is invalid
            if f==0: #when entry is valid
                y='X'
